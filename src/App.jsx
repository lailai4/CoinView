...
/** 纯前端行情 LITE（CoinGecko；失败自动演示；可选 API Key） */
const API = { coingecko: "https://api.coingecko.com/api/v3" };
// 可选：在浏览器控制台设置 window.CG_API_KEY = "你的Key"
function cgHeaders() {
...
export default function MarketsLiteApp() {
  const [coins, setCoins] = React.useState([]);
  const [selected, setSelected] = React.useState(null);
  const [ohlc, setOhlc] = React.useState([]);
  const [days, setDays] = React.useState(() => {
    try { return localStorage.getItem("wc_days") || "7"; } catch { return "7"; }
  });
  const [query, setQuery] = React.useState("");
  const [loading, setLoading] = React.useState(false);
  const [error, setError] = React.useState("");
  const [offlineDemo, setOfflineDemo] = React.useState(false);
  const [selfTestPassed, setSelfTestPassed] = React.useState(null);
  ...
  React.useEffect(() => {
    try {
      ...
      const fn = formatNumber(1_234_567) === "1.23M";
      const tm = typeof formatTime(Date.now()) === "string";
      setSelfTestPassed(asc && hasBounds && fn && tm);
    } catch { setSelfTestPassed(false); }
  }, []);

  React.useEffect(()=>{ try{ localStorage.setItem("wc_days", String(days)); }catch{} }, [days]);
  ...
  const fetchCoins = React.useCallback(async () => {
    ...
    try {
      ...
      if (r.ok && Array.isArray(r.data)) {
        ...
        setCoins(demo); if (!selected) setSelected(demo[0]);
        setOfflineDemo(true); setLastUpdated(Date.now());
      }
    } finally { setLoading(false); }
  }, [selected]);

  React.useEffect(()=>{ fetchCoins(); }, [fetchCoins]);

  React.useEffect(()=>{
    if (!autoRefresh) return;
    const id = setInterval(()=>{ fetchCoins(); if (selected) fetchOHLC(selected.id, days); }, 30_000);
    return ()=> clearInterval(id);
  }, [autoRefresh, fetchCoins, selected, days]);

  const fetchOHLC = React.useCallback(async (coinId, d) => {
    setOhlc([]);
    ...
    if (r.ok && Array.isArray(r.data) && r.data.length) {
      ...
      const base = Date.now();
      let price = selected?.current_price || 65000;
      const arr = Array.from({ length: 48 }, (_, i) => {
        const ts = base - (48 - i) * 60 * 60 * 1000;
        const vol = (Math.random()*0.02 - 0.01) * price;
        const open = price; const close = price + vol;
  ...
  React.useEffect(()=>{ if (selected) fetchOHLC(selected.id, days); }, [selected, days, fetchOHLC]);

  const filtered = React.useMemo(()=>{
    const q = query.trim().toLowerCase(); if (!q) return coins;
    return coins.filter(c=> c.name.toLowerCase().includes(q) || c.symbol.toLowerCase().includes(q));
  }, [coins, query]);

  const fmtMoney = (v) => "$" + new Intl.NumberFormat("en-US", { maximumFractionDigits: 2 }).format(v ?? 0);
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-0 backdrop-blur supports-[backdrop-filter]:bg-neutral-900/60 z-10">
              <div className="flex items-center gap-3 py-3">
                <input value={query} onChange={e=>setQuery(e.target.value)} placeholder="搜索币种（BTC, ETH...）" className="w-full rounded-2xl bg-neutral-900 border border-neutral-800 px-4 py-2 outline-none focus:ring-2 focus:ring-cyan-500" />
              </div>
            </div>
                ...
                <button key={c.id} onClick={()=>setSelected(c)} className={cls("w-full flex items-center gap-3 p-3 text-left hover:bg-neutral-900/70 transition", selected?.id===c.id && "bg-neutral-900/70") }>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="ml-auto font-mono">{fmtMoney(c.current_price)}</span>
                    </div>
    ...
    <div className="bg-gradient-to-b from-cyan-700/20 to-transparent border-b border-neutral-800">
      <div className="mx-auto max-w-7xl px-4 py-5 flex items-center gap-3">
        ...
        <div>
          ...
          <div className="text-xs text-neutral-400">无登录 · 无后端 · 可直接部署到静态托管</div>
        </div>
        <div className="ml-auto flex items-center gap-3">
          {typeof selfTestPassed === "boolean" && (
            <span className={`text-xs px-2 py-1 rounded-full border ${selfTestPassed?"border-green-500/40 text-green-400":"border-red-500/40 text-red-400"}`}>{selfTestPassed?"自检通过":"自检失败"}</span>
          )}
          ...
          <button onClick={onRefresh} className="rounded-xl border border-neutral-700 px-3 py-2 hover:bg-neutral-800 active:scale-95 transition text-sm" disabled={loading}>{loading?"刷新中…":"刷新"}</button>
        </div>
      </div>
...
function CandleChart({ data, height = 360, padding = { left: 50, right: 20, top: 20, bottom: 28 } }) {
  ...
  React.useEffect(()=>{ const ro = new ResizeObserver((es)=>{ for (const e of es) setWidth(e.contentRect.width); }); if (ref.current) ro.observe(ref.current); return ()=> ro.disconnect(); }, []);

  const { candles, min, max, ticks } = React.useMemo(()=>{
